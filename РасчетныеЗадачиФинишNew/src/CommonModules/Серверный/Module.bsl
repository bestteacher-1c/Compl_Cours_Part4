...
Процедура ПроведениеКадровыхДокументовОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Источник.Движения.ПлановыеУдержания__.Записывать = Истина;
	//
	...
	ЗаполнитьНаборЗаписей_ПлановыеУдержания_(Источник);
...
Процедура ЗаполнитьНаборЗаписей_ПлановыеУдержания_(Источник)
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ПриказОбУвольнении") Тогда
		...
		|ИЗ
		|	РегистрСведений.ПлановыеУдержания__.СрезПоследних(&ДатаСобытия, ФизическоеЛицо = &ФизическоеЛицо) КАК ПлановыеУдержанияСрезПоследних
		|ГДЕ
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Движение = Источник.Движения.ПлановыеУдержания__.Добавить();
