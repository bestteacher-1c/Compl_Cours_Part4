...
Процедура РассчитатьРесурсы_ОсновныеНачисления(НаборЗаписей, ДополнительныеПараметры) Экспорт 
	...
	|ИЗ
	|	РегистрСведений.ЗначенияПоказателейРасчетаНачислений.СрезПоследних(&ПериодРегистрации, (ФизическоеЛицо, ВидРасчета) В
	|		(ВЫБРАТЬ
	|			ВТДвиженияСПриоритетом.ФизическоеЛицо,
	|			ВТДвиженияСПриоритетом.ВидРасчета
	|		ИЗ
	|			ВТДвиженияСПриоритетом КАК ВТДвиженияСПриоритетом)) КАК ЗначенияПоказателейРасчетаНачисленийСрезПоследних
	|ГДЕ
	...
	|ГДЕ
	|	ЗначенияПоказателейРасчетаНачислений.Значение > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Выбор
	|		Когда ВТДвиженияСПриоритетом.ВидРасчета.СпособРасчета = Значение(Перечисление.СпособыРасчета.ПоСреднемуЗаработку)
	|			Тогда &ПроцентВыплаты
	|		Иначе ВТЗначенияПоказателей.Значение
	|	Конец КАК Значение,
	|	ВТЗначенияПоказателей.Показатель.Идентификатор КАК Идентификатор
	|ИЗ
	|	ВТДвиженияСПриоритетом КАК ВТДвиженияСПриоритетом
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗначенияПоказателей КАК ВТЗначенияПоказателей
	|		ПО ВТЗначенияПоказателей.ФизическоеЛицо = ВТДвиженияСПриоритетом.ФизическоеЛицо
	|		И ВТЗначенияПоказателей.ВидРасчета = ВТДвиженияСПриоритетом.ВидРасчета
	|		И ВТЗначенияПоказателей.Период = ВТДвиженияСПриоритетом.ПериодДействияНачало
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет,
	|	НомерСтроки
	...
	Запрос.УстановитьПараметр("ПериодРегистрации", ДополнительныеПараметры.ПериодРегистрации);

	Если ДополнительныеПараметры.Свойство("ПроцентВыплаты") Тогда
		Запрос.УстановитьПараметр("ПроцентВыплаты", ДополнительныеПараметры.ПроцентВыплаты);
	КонецЕсли;
...
Процедура РассчитатьРесурсы_ДополнительныеНачисления(НаборЗаписей, ДополнительныеПараметры) Экспорт
	...
	Пока ВыборкаПриоритет.Следующий() Цикл
		...
		|ПОМЕСТИТЬ ВТБазаВОсновныхНачислениях
		|ИЗ
		|	РегистрРасчета.ДополнительныеНачисления.БазаОсновныеНачисления(&ИЗмерения_ДополнительныеНачисления,
		|		&Измерения_ОсновныеНачисления,, Регистратор = &Регистратор
		|	И ВидРасчета.ТребуетСбораБазы = ИСТИНА
		|	И НомерСтроки В
		|		(ВЫБРАТЬ
		|			ВТЗаписиДляРасчета.НомерСтроки
		...
		|ИЗ
		|	РегистрРасчета.ДополнительныеНачисления.БазаДополнительныеНачисления(&ИЗмерения_ДополнительныеНачисления,
		|		&ИЗмерения_ДополнительныеНачисления,, Регистратор = &Регистратор
		|	И ВидРасчета.ТребуетСбораБазы = ИСТИНА
		|	И НомерСтроки В
		|		(ВЫБРАТЬ
		|			ВТЗаписиДляРасчета.НомерСтроки
		...
		|ВЫБРАТЬ
		|	ВТЗаписиДляРасчета.НомерСтроки КАК НомерСтроки,
		|	ЕСТЬNULL(ВТБазаВОсновныхНачислениях.РезультатБаза, 0) + ЕСТЬNULL(ВТБазаВДополнительныхНачислениях.РезультатБаза,
		|		0) КАК РезультатБаза,
		|	ЕСТЬNULL(ВТБазаВОсновныхНачислениях.ОтработаноДнейБаза, 0) КАК ОтработаноДнейБаза,
...
Процедура РассчитатьРесурсы_Удержания(НаборЗаписей, ДополнительныеПараметры) Экспорт
	...
	|ПОМЕСТИТЬ ВТЗначенияПоказателей
	|ИЗ
	|	РегистрСведений.ЗначенияПоказателейРасчетаУдержаний.СрезПоследних(&ПериодРегистрации, (ФизическоеЛицо, ВидРасчета) В
	|		(ВЫБРАТЬ
	|			ВТДвиженияСПриоритетом.ФизическоеЛицо,
	|			ВТДвиженияСПриоритетом.ВидРасчета
	|		ИЗ
	|			ВТДвиженияСПриоритетом КАК ВТДвиженияСПриоритетом)) КАК ЗначенияПоказателейРасчетаУдержанийСрезПоследних
	|ГДЕ
	...
	|ГДЕ
	|	ЗначенияПоказателейРасчетаУдержаний.Значение > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТДвиженияСПриоритетом.ВидРасчета.Формула КАК Формула,
	|	ВТЗначенияПоказателей.Показатель,
	|	ВТЗначенияПоказателей.Значение,
	|	ВТЗначенияПоказателей.Показатель.Идентификатор КАК Идентификатор
	|ИЗ
	|	ВТДвиженияСПриоритетом КАК ВТДвиженияСПриоритетом
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗначенияПоказателей КАК ВТЗначенияПоказателей
	...
	Пока ВыборкаПриоритет.Следующий() Цикл
		...
		|ПОМЕСТИТЬ ВТБазаВОсновныхНачислениях
		|ИЗ
		|	РегистрРасчета.Удержания.БазаОсновныеНачисления(&Измерения_Удержания, &Измерения_ОсновныеНачисления,,
		|		Регистратор = &Регистратор
		|	И ВидРасчета.ТребуетСбораБазы = ИСТИНА
		|	И НомерСтроки В
		|		(ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТБазаВДополнительныхНачислениях
		|ИЗ
		|	РегистрРасчета.Удержания.БазаДополнительныеНачисления(&Измерения_Удержания, &Измерения_ДополнительныеНачисления,,
		|		Регистратор = &Регистратор
		|	И ВидРасчета.ТребуетСбораБазы = ИСТИНА
		|	И НомерСтроки В
		|		(ВЫБРАТЬ
...
Процедура РасчетРесурсов(ВыборкаДетальныеЗаписи, Выборка_ДанныеГрафика_База, Запись)
	...
	Если  ВыборкаДетальныеЗаписи.СпособРасчета = Перечисления.СпособыРасчета.ПоСреднемуЗаработку Тогда
		...
		Если ОтработалЗаТриМесяца = 0 Тогда
			...
			Запись.Результат = 19242 / Выборка_ДанныеГрафика_База.ЗначениеДниПериодДействия * ДнейБолел; 
		...
		ПроцентВыплаты = ВыборкаДетальныеЗаписи.Значение;
	...
	ИначеЕсли ВыборкаДетальныеЗаписи.СпособРасчета = Перечисления.СпособыРасчета.ПоФормуле Тогда
		...
		Показатель = ВыборкаДетальныеЗаписи.Значение;
...
Процедура ПереРассчитатьРесурсы_ОсновныеНачисления(НаборЗаписей) Экспорт 
	...
	|			И (ВТПерерасчет.ФизическоеЛицо = ОсновныеНачисления.ФизическоеЛицо)
	|ГДЕ
	|	ОсновныеНачисления.Регистратор = &Регистратор
	|;
	|
