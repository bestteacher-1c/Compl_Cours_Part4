&НаКлиенте
Процедура ОсновныеНачисленияФизическоеЛицоПриИзменении(Элемент)

	ТекДанные = Элементы.ОсновныеНачисления.ТекущиеДанные;

	ОсновныеНачисленияФизическоеЛицоПриИзмененииНаСервере(Объект.ПериодРегистрации,
	ТекДанные.ФизическоеЛицо, ТекДанные.Подразделение, ТекДанные.Должность);

КонецПроцедуры

&НаСервере
Процедура ОсновныеНачисленияФизическоеЛицоПриИзмененииНаСервере(Период, ФизическоеЛицо, Подразделение, Должность)
	...
	|ИЗ
	|	РегистрСведений.Сотрудники.СрезПоследних(&Период, ФизическоеЛицо = &ФизическоеЛицо) КАК СотрудникиСрезПоследних";
...
Процедура РассчитатьРесурсы(Команда)
	...
	Если Объект.Проведен = Истина Тогда
		...
		Новый ОписаниеОповещения("РассчитатьРесурсыЗавершение", ЭтотОбъект),
			"Рассчитывать можно только НЕПРОВЕДЕННЫЙ документ. Отменить проведение?", РежимДиалогаВопрос.ДаНет, 30,
			КодВозвратаДиалога.Да, "Внимание! Документ проведен.", КодВозвратаДиалога.Да);
...
Процедура ЗаполнитьТЧ(Команда)
	...
	Новый ОписаниеОповещения("ЗаполнитьТЧЗавершение", ЭтотОбъект),
		"Табличные части, если они заполнены, будут очищены. Продолжить!", РежимДиалогаВопрос.ДаНет, 30,
		КодВозвратаДиалога.Да, "Внимание", КодВозвратаДиалога.Да);
КонецПроцедуры
