...
Процедура РассчитатьРесурсы_Начисления()
	...
	Пока ВыборкаПриоритет.Следующий() Цикл
		...
		|	НачисленияБазаНачисления.ФизическоеЛицо КАК Начальник
		|ПОМЕСТИТЬ ВТБазаВНачислениях
		|ИЗ
		|	РегистрРасчета.Начисления.БазаНачисления(
		|			&Измерения_Начисления,
		|			&Измерения_Начисления,
		|			&Поля_Разрезы,
		...
		|			И (ВТБазаВНачислениях.Начальник <> ВТБазаВНачислениях.ФизическоеЛицоРазрез)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТЗаписиДляРасчета.НомерСтроки
		|
		...
		Поля_Разрезы = Новый Массив;
		Поля_Разрезы.Добавить("ФизическоеЛицо");
		...
		Запрос.УстановитьПараметр("Поля_Разрезы", Поля_Разрезы);
